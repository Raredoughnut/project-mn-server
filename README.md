...
인증 흐름, CSRF, 엔드포인트별 요청/응답, 아직 값이 비어 있는 필드는 **[docs/api.md](docs/api.md)**
에 정리해 두었다. 프론트엔드 작업자에게는 그 문서를 건네면 된다.

API 를 고칠 때 그 문서도 같은 커밋에서 함께 고칠 것 — 특히 세션·CSRF 처럼 프론트를 깨뜨리는
변경은 코드만 바꾸면 상대는 알 방법이 없다.

### 팀원이 백엔드를 로컬에서 띄우려면

**[docs/local-setup.md](docs/local-setup.md)** 를 건네면 된다. 실행 절차부터 데이터를 채우는
방법(직접 임포트 / DB 덤프), 화면별로 무엇이 있어야 보이는지, 막혔을 때 볼 곳까지 담았다.

`GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` 은 저장소에 없으므로 **별도 채널(비밀번호 관리자 등)로
직접 전달해야 한다.** 저장소에 커밋하지 말 것.
